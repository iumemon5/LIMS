      ...
      return (
         <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
               {worksheets.length === 0 ? (
                  <div className="col-span-full py-16 flex flex-col items-center justify-center text-slate-400 bg-slate-50 border-2 border-dashed border-slate-300 rounded-2xl group hover:border-blue-400 hover:bg-blue-50/50 transition-colors">
                     <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center shadow-sm mb-4 group-hover:scale-110 transition-transform">
                        <FileSpreadsheet size={32} className="text-slate-400 group-hover:text-blue-500" />
                     </div>
                     <h3 className="text-lg font-bold text-slate-900">No active worksheets</h3>
                     <p className="text-sm font-medium text-slate-500 mb-6 max-w-xs text-center">Create a new batch worksheet to manage sample processing and entering results.</p>
                     <button
                        onClick={() => setView('create')}
                        className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2.5 rounded-xl font-bold text-sm shadow-md hover:shadow-lg flex items-center gap-2 transition-all active:scale-95"
                     >
                        <Plus size={18} /> Create New Worksheet
                     </button>
                  </div>
               ) : (
                  worksheets.slice().reverse().map(ws => (
                     ...
                     >
                        <div className="flex justify-between items-start mb-4">
                           ...
                           <span className={`px-2 py-1 rounded text-[10px] font-black uppercase ${ws.status === 'Open' ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-500'
                              }`}>
                              {ws.status}
                           </span>
                        </div>
                                       ...
                                       return (
                                          <td key={col.code} className="px-4 py-3">
                                             <input
                                                ...
                                                className={`w-full border rounded px-2 py-1 text-sm font-bold outline-none focus:ring-2 focus:ring-blue-500 ${analysis.result ? 'border-green-300 bg-green-50 text-green-800' : 'border-slate-200'
                                                   }`}
                                                placeholder="--"
               ...
               <div className="w-64 border-r border-slate-200 bg-slate-50 p-4 flex flex-col gap-2">
                  ...
                  {departments.map(dept => (
                     <button
                        ...
                        className={`w-full text-left px-4 py-3 rounded-xl text-sm font-bold flex justify-between items-center transition-all ${selectedDeptId === dept.id
