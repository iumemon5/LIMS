...
const Layout: React.FC<LayoutProps> = ({ children, activeTab, setActiveTab }) => {
  ...
  useEffect(() => {
    ...
    const down = (e: KeyboardEvent) => {
      ...
      if (e.key === 'd' && e.altKey) { e.preventDefault(); setActiveTab('dashboard'); } // Alt+D for Dashboard
    };
  ...
  return (
    <div className="flex h-screen overflow-hidden text-slate-900 bg-slate-50 font-sans selection:bg-blue-100 selection:text-blue-700">
              ...
              {group.items.map(item => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1 group ${isActive
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden bg-slate-50/50">
        ...
        <ShortcutsHelp isOpen={showHelpModal} onClose={() => setShowHelpModal(false)} />
      </main>
    </div>
  );
};
