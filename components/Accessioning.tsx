...
const Accessioning: React.FC = () => {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Escape: Reset/Close
      if (e.key === 'Escape') {
        if (isPatientModalOpen) setIsPatientModalOpen(false);
        // If a dropdown is open or search has text, specific handlers will likely catch it, 
        // but global ESC can also clear selections if needed.
        if (createdRequest) setCreatedRequest(null);
      }
    };

    window.addEventListener('keydown', handleKeyDown);
    return () => window.removeEventListener('keydown', handleKeyDown);
  }, [isPatientModalOpen, createdRequest, selectedPatient, selectedTests, subtotal, discountVal, paidVal, sampleType, referrer, priority]);

  // Reset indices on search change
  useEffect(() => setActivePatientIndex(-1), [patientSearch]);
  useEffect(() => setActiveTestIndex(-1), [testSearch]);
